...
* sentenceConfig.setMaxWordLength(20)(package com.wlld.myjecs.bean)，设置最大语句长度（超出该长度的语句将被截断，只处理最大长度之内的）。该数值与模型绑定，若修改该数值除了词嵌入模型外，其他都需要重新训练。该数值越大运算速度越慢，需要样本量越大，所以请根据各自业务实际情况修改。
### 演示数据sql脚本及静态资源位置
